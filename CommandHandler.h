...
#define EEPROM_STORED_COMMAND_LOCATION \
  EEPROM_STORED_COMMAND_FLAG_LOCATION + sizeof(bool)
#endif
...
class ParameterLookup {
 public:
  ...
  // The buffer pointed to by commandStr must be at least COMMAND_SIZE_MAX in
  // length
  ...
  const char* operator[](int idx) const {
    if (idx == -1) {
      ...
      if (_stringHasNULLS) restoreSpaces();
      ...
      if (!_stringHasNULLS) subSpacesForNULL();
...
// and execute it
//
// This object can hold <size> commands where <size> is defined at compile time,
// e.g.
//
...
// #define EEPROM_DISABLED
// The user must call `executeStartupCommands()` in their code once they are
// ready for EEPROM commands to be executed
...
class CommandHandler {
 ...
 public:
  ...
  // Register a command
  // This version is deprecated because it involves storing the strings in
  // memory for its calling which defeats the point of hashes!
  CommandHandlerReturn registerCommand(const char* command,
  ...
  // Register a command
  // Use this version instead. To calculate the hash, use
  // COMMANDHANDLER_HASH(cmd) e.g.
  ...
  // Add a char from the serial connection to be processed and added to the
  // queue Returns BUFFER_FULL if buffer is full and char wasn't added
  CommandHandlerReturn addCommandChar(const char c) {
    ...
    // else c is a normal char, so add it to the buffer
      ...
      if (_command_too_long || _bufferLength >= COMMAND_SIZE_MAX - 1) {
        // Command was too long! Set the `_command_too_long` flag to chuck away
        // all subsequent chars until next newline
  ...
  // This command should not include newlines: it will be copied verbatim into
  // the buffer and then executed as a normal command would be Multiple commands
  // can be seperated by ';' chars Max length is EEPROM_SIZE_MAX - 2 (1 char to
  // append a newline, 1 for the null term) Returns false on fail
  ...
  // the buffer and then executed as a normal command would be Multiple commands
  // can be seperated by ';' chars Max length is EEPROM_SIZE_MAX - 2 (1 char to
  // append a newline, 1 for the null term) Returns CommandHandlerReturn to
  ...
  bool wipeStartupCommand() {
    // There is no need to wipe the command from the EEPROM: we simply set the
    // flag to false
  ...
  void getStartupCommand(char* buf) {
    ...
    // There should be a bool stored in EEPROM_STORED_COMMAND_FLAG_LOCATION if
    // this program has run before It will tell us if there's a command to be
    // read or not Read it as a byte though, since the memory location will be
    // 0xFF if it has never been written to We only want to use it if it's
    // exactly a bool
    char fromEEPROM;
    ...
    if (fromEEPROM == (char)true) {
      ...
      // At most, go to the end of the buffer - 1, to leave space for a null
      // terminator
  ...
  CommandHandlerReturn executeStartupCommands() {
    ...
    if (fromEEPROM != (char)true) {
      CONSOLE_LOG_LN(
          F("CommandHandler::executeStartupCommands: No command stored"));
      return CommandHandlerReturn::NO_COMMAND_WAITING;
    }

    // Command is waiting, so queue it one char at a time
    // If we reach a newline, commandWaiting() will flag "true": execute the
    // command If we reach a NULL, end If we've read COMMAND_SIZE_MAX - 2 bytes
    // from EEPROM, stop and add a newline + NULL
 ...
 private:
  ...
  //
  // Its maximum size is determined at compile time by the `size` template
  // argument
 ...
 private:
  ...
  class CommandLookup {
   public:
    CommandLookup() : _commandsIdx(0) {}

    // Add a new command to the list, calculating its hash at runtime
    // (deprecated)
    CommandHandlerReturn registerCommand(const char* command,
                                         int num_of_parameters,
                                         commandFunction* pointer_to_function) {
    ...
    CommandHandlerReturn registerCommand(long keyHash, int num_of_parameters,
      ...
      // Set up a struct containing the number of params and a pointer to the
      // function
      dataStruct d;
    ...
    // Search the list of commands for the given command and execute it with the
